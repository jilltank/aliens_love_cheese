...
# Starts with a small search radius and then widens the search until it returns something. Note: sometimes you will still not get any returns. Sorry, empty plains states.
    ...
    if nacho_bases.empty?
      nacho_bases = HTTParty.get("http://www.databaseofnachos.com/api/search_within?nelat=#{ne_lat+0.46}&nelng=#{ne_long+0.46}&swlat=#{sw_lat-0.46}&swlng=#{sw_long-0.46}&offset=0")
    end
    if nacho_bases.empty?
      nacho_bases = HTTParty.get("http://www.databaseofnachos.com/api/search_within?nelat=#{ne_lat+0.66}&nelng=#{ne_long+0.66}&swlat=#{sw_lat-0.66}&swlng=#{sw_long-0.66}&offset=0")
    end
    if nacho_bases.empty?
      nacho_bases = HTTParty.get("http://www.databaseofnachos.com/api/search_within?nelat=#{ne_lat+0.91}&nelng=#{ne_long+0.91}&swlat=#{sw_lat-0.91}&swlng=#{sw_long-0.91}&offset=0")
    end
    # if nacho_bases.empty?
